...
function startExpress() {
    return new Promise((resolve, reject) => {
        ...
        const serverPath = path.join(__dirname, '..', 'servers', 'express-server.js');
        ...
        server.stdout.on('data', (data) => {
            ...
            if (output.includes(`listening on port ${EXPRESS_PORT}`)) {
                console.log(`${GREEN}[Express]${RESET} Server ready on port ${EXPRESS_PORT}\n`);
                resolve(server);
            }
        });
...
async function runBenchmarks() {
    ...
    try {
        ...
        for (const scenario of scenarios) {
            ...
            // Display results
            console.log(`  ${GREEN}Requests/sec:${RESET} ${formatNumber(metrics.requestsPerSec)}`);
            console.log(`  ${GREEN}Avg Latency:${RESET}  ${formatNumber(metrics.avgLatency)}ms`);
            console.log(`  ${GREEN}P50 Latency:${RESET}  ${formatNumber(metrics.p50Latency)}ms`);
            console.log(`  ${GREEN}P99 Latency:${RESET}  ${formatNumber(metrics.p99Latency)}ms`);
            console.log(`  ${GREEN}Total Reqs:${RESET}   ${formatNumber(metrics.totalRequests)}\n`);
        }

        // Summary table
        console.log(`\n${CYAN}╔════════════════════════════════════════════╗${RESET}`);
        console.log(`${CYAN}║           Benchmark Summary                ║${RESET}`);
        console.log(`${CYAN}╚════════════════════════════════════════════╝${RESET}\n`);
        ...
        console.log(`${GREEN}Average Throughput:${RESET} ${formatNumber(avgRps)} req/sec`);
        ...
        const filename = path.join(resultsDir, `express-baseline-${timestamp}.json`);
